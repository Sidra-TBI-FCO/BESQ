...
for (i in 2:N.sets) {
  ...
  TS.EventFree = Survival_data[Survival_data[, Outcome] == "0", c(Outcome, paste0(Outcome, ".time"), Ethnicity_variable, "ajcc_pathologic_tumor_stage")]
  colnames(TS.EventFree) = c("Status","Time", "Group", "Stage")
  TS.EventFree$Time = as.numeric(as.character(TS.EventFree$Time))
  TS.EventFree$Time[TS.EventFree$Time > Y] = Y
  
  TS.EventOccured = Survival_data[Survival_data[, Outcome] == "1", c(Outcome, paste0(Outcome, ".time"), Ethnicity_variable, "ajcc_pathologic_tumor_stage")]
  colnames(TS.EventOccured) = c("Status","Time", "Group", "Stage")
...
multivariate_rev = coxph(formula = Surv(Time, Status) ~ Group + Stage, data = TS.Surv)
