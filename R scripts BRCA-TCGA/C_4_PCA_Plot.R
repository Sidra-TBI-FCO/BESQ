...
#PCA
if(Type == "Normalized"){
  Expression.matrix = t(filtered.norm.RNAseqData)
}
...
scores$ICR = clustering$HML.ICR.Cluster[match(rownames(Expression.matrix), rownames(clustering))]

scores$ICR[scores$ICR == "ICR High"] = "red"
scores$ICR[scores$ICR == "ICR Medium"] = "green"
...
ggplot(scores, aes(x=PC1, y=PC2, color = ICR)) +
  geom_point(size=1.5, aes(col=ICR))+
  #geom_text_repel() +
  #scale_colour_brewer(palette = "Set2")+
  scale_color_manual(name="ICR", values = c("red","green", "blue"))+
