...
dir.create("./Figures/Kaplan_meiers/C39.5_Aneuploidy_score", showWarnings = FALSE)
...
plot_df = data.frame(Patient = Survival_data$bcr_patient_barcode,
                     ...
                     Aneuploidy_score = NA,
                     Aneuploidy_score_category = NA)

plot_df$Aneuploidy_score = pheno.data$Core56.DNA.Alteration_Aneuploidy.Score[match(plot_df$Patient,
                                                                                   pheno.data$Patient.ID)]

plot_df = plot_df[-which(is.na(plot_df$Aneuploidy_score)),]

aneuploidy_patients = as.character(plot_df$Patient)

save(aneuploidy_patients, file = "./Figures/Kaplan_meiers/C39.5_Aneuploidy_score/aneuploidy_patients.Rdata")

plot_df$Aneuploidy_score_category[which(plot_df$Aneuploidy_score < cutoff)] = "Aneuploidy Low"
plot_df$Aneuploidy_score_category[which(plot_df$Aneuploidy_score >= cutoff)] = "Aneuploidy High"
plot_df$OS = as.character(plot_df$OS)

Y = Surv_cutoff_years * 365
TS.EventFree = plot_df[which(plot_df$OS == "0"), c("OS", "OS.Time", "Aneuploidy_score_category", "Aneuploidy_score")]
colnames(TS.EventFree) = c("Status","Time", "Group", "Continuous")
TS.EventFree$Time = as.numeric(as.character(TS.EventFree$Time))
TS.EventFree$Time[TS.EventFree$Time > Y] = Y

TS.EventOccured = plot_df[which(plot_df$OS == "1"), c("OS", "OS.Time", "Aneuploidy_score_category", "Aneuploidy_score")]
colnames(TS.EventOccured) = c("Status","Time", "Group", "Continuous")
...
TS.Surv[,"Group"] = factor(TS.Surv[,"Group"], levels = c("Aneuploidy High", "Aneuploidy Low"))

# Check this!!
##TS.Surv[,"Group"] = relevel(TS.Surv[,"Group"], "ICR Pathway High")
mHR = coxph(formula = msurv ~ TS.Surv[,"Group"],data = TS.Surv, subset = TS.Surv$Group %in% c("Aneuploidy High", "Aneuploidy Low"))
...
# plots
png(paste0("./Figures/Kaplan_meiers/C39.5_Aneuploidy_score//Kaplan_Meijer_by_Aneuploidy_score_in_", IMS ,
...
uni_variate = coxph(formula = Surv(Time, Status) ~ Continuous, data = TS.Surv)
summary = summary(uni_variate)
