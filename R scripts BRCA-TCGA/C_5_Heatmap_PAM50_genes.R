...
col_fun = circlize::colorRamp2(c(-4.5,-0.1,0,0.1, 4), c("blue", "#EAE7EF","white","#FAD9D0", "red"))

for (i in 1:length(Subtypes)){
  subtype = Subtypes[i]
  assign(paste0("mat", i), Expression.matrix.z[, which(substring(colnames(Expression.matrix.z), 1, 12) %in% 
                                       Clinical_data_ann$bcr_patient_barcode[which(Clinical_data_ann$IMS == subtype)])])
  assign(paste0("ha_column", i), HeatmapAnnotation(df = data.frame(IMS_PAM50 = Clinical_data_ann$IMS[which(Clinical_data_ann$IMS == subtype)]),
...
dir.create("./Figures/Heatmaps/PAM50_Heatmaps/", showWarnings = FALSE)
png(paste0("./Figures/Heatmaps/PAM50_Heatmaps/PAM50_Heatmap_new.png"), res = 600, width = 10, height = 10, units = "in")
plot = Heatmap(mat1,
