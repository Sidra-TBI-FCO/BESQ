...
IMS_Mathews_colors = c("BasalHer2" = "#F4A101", "BasalMyo" = "#3D64E4",
                       "BasalLumHer2" = "#92D050", "MyoLumB" = "#C720F1",
...
for (i in 1:length(Subtypes)){
  ...
  assign(paste0("mat", i), Expression.matrix.z[, which(substring(colnames(Expression.matrix.z), 1, 12) %in% 
                                                         Clinical_data_ann$bcr_patient_barcode[which(Clinical_data_ann$IMS_Mathews == subtype)])])
...
png(paste0("./Figures/Heatmaps/PAM50_Heatmaps_Mathews/PAM50_Heatmap_v4.png"), res = 600, width = 10, height = 10, units = "in")
plot = Heatmap(mat1,
