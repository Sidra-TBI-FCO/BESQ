...
max_splits = read.csv(file = paste0("./Analysis/Hossam_prediction_model/Final_SHAP_p_value_and_cutoffs/", tolower(Ethnicity) ,".csv"), stringsAsFactors = FALSE)
...
dir.create("./Figures/Kaplan_meiers/C52.1_survival_by_pathway_category", showWarnings = FALSE)

max_splits$Feature = gsub("_", " ", max_splits$Feature)
max_splits$Feature = gsub("\\:", "] ", max_splits$Feature)
if(Ethnicity == "Black"){
  max_splits$Feature= paste("[", max_splits$Feature, sep = "")
}
if(Ethnicity == "White"){
  max_splits$Feature[-which(max_splits$Feature == "ICRscore")] = paste("[", max_splits$Feature[-which(max_splits$Feature == "ICRscore")], sep = "")
}
